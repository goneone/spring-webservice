...
import com.kuyt1819.webservice.config.auth.dto.OAuthAttributes;
import com.kuyt1819.webservice.config.auth.dto.SessionUser;
import com.kuyt1819.webservice.domain.user.User;
import com.kuyt1819.webservice.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        //현재 로그인 진행중인 서비스를 구분하는 코드 => 네이버 로그인인지 구글 로그인인지 구분하는 코드
        String registrationId = userRequest.getClientRegistration().getRegistrationId();
        //OAuth2로그인 진행 시 키가 되는 필드 값. Primary Key와 같은 의미
